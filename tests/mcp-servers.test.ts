...
const { safeWriteFileMock, admitMcpCapabilityMock } = vi.hoisted(() => ({
  safeWriteFileMock: vi.fn(),
  admitMcpCapabilityMock: vi.fn(
    (
      _name: string,
      entry: {
        command: string;
...
vi.mock("../src/main/utils", () => ({
  ...
  profilePaths: () => ({ home: "/tmp/hermes", configFile: "config.yaml" }),
  safeWriteFile: safeWriteFileMock,
}));

vi.mock("../src/main/capability-risk-store", () => ({
  admitMcpCapability: admitMcpCapabilityMock,
}));
...
describe("MCP server config management", () => {
  beforeEach(() => {
    safeWriteFileMock.mockClear();
    admitMcpCapabilityMock.mockClear();
    admitMcpCapabilityMock.mockImplementation((_name, entry) => entry);
  });
...
`,
  ...
  it("writes newly added local MCP servers through capability admission", async () => {
    admitMcpCapabilityMock.mockReturnValueOnce({
      command: "npx",
      args: ["-y", "github-mcp"],
    ...
    expect(admitMcpCapabilityMock).toHaveBeenCalledWith(
      "github",
      {
        command: "npx",
        args: ["-y", "github-mcp"],
    ...
    expect(safeWriteFileMock).toHaveBeenCalledTimes(1);
    expect(String(safeWriteFileMock.mock.calls[0][1])).toContain(
      "enabled: false",
    );
  });
});
