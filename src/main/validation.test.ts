...
const mockedCustomEndpointKeyResolvable = vi.mocked(customEndpointKeyResolvable);
...
describe("validateChatReadiness — connection-mode awareness", () => {
  ...
  afterEach(() => {
    for (const k of ["ANTHROPIC_API_KEY", "ANTHROPIC_TOKEN"]) delete process.env[k];
  });
  ...
  it("REMOTE mode does NOT mask a NO_ACTIVE_MODEL config error", () => {
    ...
    // the model check.
    mockedGetModelConfig.mockReturnValue({ provider: "anthropic", model: "", baseUrl: "" });
    setMode({ mode: "remote", remoteUrl: "http://127.0.0.1:8642" });
