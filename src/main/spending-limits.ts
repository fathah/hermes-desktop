...
export function getSpendingCapConfig(): SpendingCapConfig {
  ...
  return {
    maxSpendingLimit:
      typeof config.maxSpendingLimit === "number"
        ? config.maxSpendingLimit
        : DEFAULT_CONFIG.maxSpendingLimit,
    spendingCapAction:
      (config.spendingCapAction as SpendingCapConfig["spendingCapAction"]) ||
      DEFAULT_CONFIG.spendingCapAction,
  };
}
 ...
 */
export function setSpendingCapConfig(
  settings: Partial<SpendingCapConfig>,
): void {
  const config = readDesktopConfig();
