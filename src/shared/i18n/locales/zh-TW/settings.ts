export default {
  ...
  httpProxy: "HTTP 代理",
  httpProxyHint: "所有連出連線的 SOCKS 或 HTTP 代理（留空則自動偵測）",
  saved: "已儲存",
  providerHint: "選擇推論供應商，或根據 API Key 自動辨識",
  customProviderHint:
    "使用任何 OpenAI 相容的介面（LM Studio、Ollama、vLLM 等）",
  modelHint: "預設模型名稱（留空則使用供應商預設值）",
  customBaseUrlHint: "OpenAI 相容的 API 位址",
  poolHint:
  ...
  modeRemoteHint: "連接到網路或雲端伺服器上的 Hermes API",
  remoteUrl: "遠端伺服器位址",
  remoteUrlHint:
  ...
  serverConfigHint:
    "你已連接到遠端 Hermes 伺服器。模型選擇、供應商 API Key 和憑證均在伺服器的 <code>~/.hermes/.env</code> 和 <code>config.yaml</code> 中管理。請在主機上編輯（例如 <code>docker exec -it hermes vi /opt/data/.env</code>）然後重新啟動容器。",
  connectionMode: "模式",
