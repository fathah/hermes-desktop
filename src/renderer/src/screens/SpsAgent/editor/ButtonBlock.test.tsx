...
describe("ButtonBlock", () => {
  it("opens the assistant panel and runs the agentPrompt on click", () => {
    ...
    // Messages now live in the active conversation tab (M3 parallel tabs).
    const activeMsgs = (): unknown[] => {
      const st = useStore.getState();
      return (
        st.conversations.find((c) => c.id === st.activeConvId)?.messages ?? []
      );
    };
    const before = activeMsgs().length;
    ...
    expect(activeMsgs().length).toBe(before + 1);
    expect(JSON.stringify(activeMsgs().at(-1))).toContain(
